...
const weatherTool = tool({
  ...
  execute: async ({ location }) =>
    backgroundResult(`The weather in ${location} is sunny.`),
});
...
const secretTool = tool({
  ...
  execute: async ({ question }) => `The answer to ${question} is 42.`,
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    recorder.current = new WavRecorder({ sampleRate: 24_000 });
    player.current = new WavStreamPlayer({ sampleRate: 24_000 });
