...
describe("CodexMcpService", () => {
  ...
  it("clears connecting state after failed connection attempts", async () => {
    const failingClient = new MockClient();
    const serviceWithoutRetries = new CodexMcpService({
    ...
    failingClient.connect.mockRejectedValueOnce(error);

    await expect(serviceWithoutRetries.connect()).rejects.toThrow(error);
    expect(failingClient.close).toHaveBeenCalledTimes(1);

    failingClient.connect.mockResolvedValueOnce();

    await expect(serviceWithoutRetries.connect()).resolves.toBeUndefined();
    expect(failingClient.connect).toHaveBeenCalledTimes(2);
  });
